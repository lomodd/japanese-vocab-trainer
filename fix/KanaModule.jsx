...
export default function KanaModule() {
  ...
  const displayData = useMemo(() => (isHiragana ? hiraganaData : katakanaData), [isHiragana]);
    ...
    <div className="p-4">
      ...
      <div className="flex flex-wrap gap-3 items-center justify-center mb-4">
        <div className="inline-flex rounded-full bg-zinc-800/40 p-1">
          <button
            className={`px-4 py-2 rounded-full font-medium transition ${isHiragana ? "bg-gradient-to-r from-blue-500 to-purple-500 text-white shadow" : "text-gray-300 hover:text-white"}`}
            onClick={() => {
              setIsHiragana(true);
              endPractice();
            }}
                ...
                <tr key={rowIndex}>
                  ...
                  {row.map((item, cellIndex) => (
                    <td
                      ...
                      onClick={() => {
                        if (!item || !item.kana) return;
                        // 进入练习并把该项设置为当前题
                        const flat = displayData.flat().filter(i => i && i.kana);
                        const idx = flat.findIndex(i => i.kana === item.kana && i.roma === item.roma);
                        if (idx >= 0) {
                          setPracticeList(flat);
                    ...
                    >
                      <div className="flex flex-col items-center justify-center">
                        ...
                        <span className="text-xs text-gray-500 mt-1">{item.roma}</span>
                      </div>
                    </td>
                  ))}
                </tr>
        ...
        <div className="mt-4 bg-white/95 rounded-2xl p-6 max-w-xl mx-auto shadow-md">
          <div className="flex flex-col sm:flex-row items-center justify-between mb-4 gap-3">
            <div className="text-center sm:text-left">
              <div className="text-4xl sm:text-5xl font-bold">{current ? current.kana : "—"}</div>
            </div>
